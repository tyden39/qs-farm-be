...
export class ZoneSensorConfigService {
  ...
  async createConfig(
    zoneId: string,
    dto: CreateZoneSensorConfigDto,
  ): Promise<ZoneSensorConfig> {
  ...
  async updateConfig(
    zoneId: string,
    id: string,
    dto: UpdateZoneSensorConfigDto,
  ): Promise<ZoneSensorConfig> {
    const config = await this.configRepo.findOne({ where: { id, zoneId } });
    if (!config)
      throw new NotFoundException(`Zone sensor config ${id} not found`);
  ...
  async createThreshold(
    configId: string,
    dto: CreateZoneThresholdDto,
  ): Promise<ZoneThreshold> {
    const config = await this.configRepo.findOne({ where: { id: configId } });
    if (!config)
      throw new NotFoundException(`Zone sensor config ${configId} not found`);
    const threshold = this.thresholdRepo.create({
      ...dto,
      zoneSensorConfigId: configId,
    });
  ...
  async updateThreshold(
    configId: string,
    id: string,
    dto: UpdateZoneThresholdDto,
  ): Promise<ZoneThreshold> {
    const threshold = await this.thresholdRepo.findOne({
      where: { id, zoneSensorConfigId: configId },
    });
    if (!threshold)
      throw new NotFoundException(`Zone threshold ${id} not found`);
    const config = await this.configRepo.findOne({ where: { id: configId } });
